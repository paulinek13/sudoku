...
// < VARIABLES ASSOCIATED WITH GENERATING SUDOKU
...
#define COLOR_REGION al_map_rgb(132, 132, 0)
...
void DrawText(ALLEGRO_FONT *font, float width, float height, float pos_x, float pos_y,
			  int centre, ALLEGRO_COLOR color, std::string text)
{
	...
	al_use_transform(&t);
	al_draw_textf(font, color, pos_x, centre ? pos_y - (text_height / 2) : pos_y, centre, "%s", text.c_str());
...
bool SaveSudoku(int sudoku[SIZE][SIZE], std::string name)
{
	...
	file.open("data//saved_sudoku//" + file_name);
	if(file.is_open())
	{
		for(int x = 0; x < SIZE; ++x)
		{
...
void SwappingGenerator(int sudoku[SIZE][SIZE], int clues)
{
	...
	do
	{
		...
		while(num_swap)
		{
			...
			{
				case 0:
					SwapRows(sudoku, rand() % SIZE, rand() % SIZE);
					break;
				case 1:
					SwapRowsGroup(sudoku, rand() % 3, rand() % 3);
					break;
				case 2:
					SwapColums(sudoku, rand() % SIZE, rand() % SIZE);
					break;
				case 3:
					SwapColumsGroup(sudoku, rand() % 3, rand() % 3);
					break;
				case 4:
					SwapRegions(sudoku, (rand() % 3) * 3, (rand() % 3) * 3, (rand() % 3) * 3, (rand() % 3) * 3);
					break;
				default:
					break;
			}
...
int main()
{
	...
	if (!timer)
		error = 3;
	ALLEGRO_FONT *font = al_load_font("resources//fonts//kenny_mini_square.ttf", 32, 0);
	if (!font)
		error = 4;
	ALLEGRO_BITMAP *bitmap_sudoku_logo = al_load_bitmap("resources//icon.png");
	if (!bitmap_sudoku_logo)
		error = 5;
	...
	if (!error)
	{
		...
		// init all boxes
		{
			// without adding this gap to cx and cy text is not centered... but why???
		...
		while (!end_app)
		{
			...
			case ALLEGRO_EVENT_TIMER:
			{
				...
				if (done)
				{
					...
					if (failed)
					{
						...
						str_output[1] = "generated successfully!";
				}
			}
			break;
			case ALLEGRO_EVENT_MOUSE_AXES:
			{
				...
				for (int y = 0; y < SIZE; ++y)
					for (int x = 0; x < SIZE; ++x)
						if (mouse_x > boxes[x][y].px1 && mouse_x < boxes[x][y].px2 && mouse_y > boxes[x][y].py1 && mouse_y < boxes[x][y].py2)
							boxes[x][y].hovered = 1;
						else
							boxes[x][y].hovered = 0;
				...
				for (int i = 0; i < NUMBER_BUTTONS; ++i)
				{
					if (mouse_x > buttons[i].px1 && mouse_x < buttons[i].px2 && mouse_y > buttons[i].py1 && mouse_y < buttons[i].py2)
					{
						buttons[i].hovered = 1;
				...
				if (mouse_x > 25.f && mouse_x < 975.f && mouse_y > 725.f && mouse_y < 775.f)
					input_box.hovered = 1;
				else
					input_box.hovered = 0;
			}
			...
			if (draw_app && al_is_event_queue_empty(event_queue))
			{
				...
				DrawText(font, 500.f, 125.f, 500.f, 50.f, 1, COLOR_DARK, "Sudoku App");
				...
				// draw sudoku
				{
					...
					// draw a rectangle for each region to easily differentiate them
					{
						...
						for (int i = 0; i < 3; ++i)
						{
							for (int j = 0; j < 3; ++j)
							{
								al_draw_rectangle(rect_px, rect_py, rect_px + 192.5f,
												  rect_py + 192.5f, COLOR_REGION, 13);
								rect_px += 198.5f;
							}
							rect_py += 198.5f;
							rect_px = 31.f;
						...
						for (int y = 0; y < SIZE; ++y)
						{
							for (int x = 0; x < SIZE; ++x)
							{
								...
								if (boxes[x][y].number != 0)
									DrawText(font, boxes[x][y].hovered ? 90.f : 80.f, boxes[x][y].hovered ? 90.f : 80.f,
											 boxes[x][y].cx - 20.f, boxes[x][y].cy - 50.f, 0, COLOR_DARK, std::to_string(boxes[x][y].number));
								if (box_active)
								{
									if (boxes[x][y].number == box_active->number)
									{
										if ((boxes[x][y].x == box_active->x) || (boxes[x][y].y == box_active->y) || (boxes[x][y].region == box_active->region))
											al_draw_rectangle(boxes[x][y].px1, boxes[x][y].py1, boxes[x][y].px2, boxes[x][y].py2,
															  COLOR_DUPLICATE_NUMBER, 5.f);
										else
											al_draw_rectangle(boxes[x][y].px1, boxes[x][y].py1, boxes[x][y].px2, boxes[x][y].py2,
															  COLOR_SAME_NUMBER, 5.f);
									}
								}
							}
						}
				...
				if (box_active)
					al_draw_filled_rectangle(box_active->px1, box_active->py1, box_active->px2, box_active->py2, COLOR_ACTIVED);
				...
				// draw input box
				{
					...
					if (input_box.active)
					{
						...
						float px = 120.f + (input_box.text.size() * 40.f);
						al_draw_filled_rectangle(px, 725.f, px + 10.f, 775.f,
												 al_map_rgba(0, 0, 0, al_get_timer_count(timer) * 20));
					}
					for (unsigned char i = 0; i < input_box.text.size(); ++i)
					{
						std::string str(1, input_box.text.at(i));
						DrawText(font, 40.f, 40.f, 120.f + (i * 40.f), 735.f, ALLEGRO_ALIGN_LEFT, COLOR_DARK, str.c_str());
					}
				}
				...
				// draw output box
				{
					...
					DrawText(font, 500.f, 30.f, 500.f, 825.f, ALLEGRO_ALIGN_CENTER, COLOR_DARK, str_output[0]);
					DrawText(font, 900.f, 70.f, 500.f, 900.f, ALLEGRO_ALIGN_CENTER, COLOR_DARK, str_output[1]);
				}
